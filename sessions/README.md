# sessions/ — Bitácora de Trabajo

Carpeta de bitácoras: **una entrada por sesión de trabajo** para que cualquier agente (humano o Claude) recupere contexto rápidamente sin re-leer todo el repo.

Nombre: `YYYY-MM-DD-<slug>.md`. Plantilla en `TEMPLATE.md`. Una entrada por sesión; actualizar al cerrar con `Estado: completada` + Next Steps. Entradas cortas — si supera 200 líneas, era más de una sesión.

## Índice rápido

Mantén actualizado el índice cronológico abajo (más reciente arriba):

- [2026-05-10 — Panel de mensualidades activas en parking (HU-047)](./2026-05-10-monthly-plans-panel.md)
- [2026-05-09 — Fix CDK Dialog: route injector en todos los modales](./2026-05-09-fix-cdk-dialog-route-injector.md)
- [2026-05-04 — Tax config + corrección fórmula IVA en facturas](./2026-05-04-tax-iva-spec.md)
- [2026-05-03 — Auditoría: duplicación, archivos > 400, componentización](./2026-05-03-audit-componentizacion.md)
- [2026-05-03 — Reports: UX rica (presets, deltas, gráficos, tab contable)](./2026-05-03-reports-ux-rich.md)
- [2026-05-02 — Siigo (Fase 11): S5 cron polling + pg_cron](./2026-05-02-siigo-fase-11-s5.md)
- [2026-05-02 — Siigo (Fase 11): S4 EF siigo-emit-invoice](./2026-05-02-siigo-fase-11-s4.md)
- [2026-05-02 — Siigo (Fase 11): S3 _shared/siigo helpers](./2026-05-02-siigo-fase-11-s3.md)
- [2026-05-02 — Siigo (Fase 11): S2 schema delta + RLS](./2026-05-02-siigo-fase-11-s2.md)
- [2026-05-01 — Siigo (Fase 11): plan + S1 specs](./2026-05-01-siigo-fase-11-plan.md)
- [2026-05-01 — DIAN FE Service: plan inicial y bootstrap](./2026-05-01-dian-fe-service-plan.md)
- [2026-04-30 — UI/UX: design system, sidebar, dashboard, login toggle](./2026-04-30-ui-ux-mejoras-diseno.md)
- [2026-04-30 — Fix: JWT hook permisos + email_change NULL](./2026-04-30-fix-jwt-hook-permissions.md)
- [2026-04-29 — Integraciones post-Fase 9 + Fase 8 (PowerSync)](./2026-04-29-integraciones-guardias.md)
- [2026-04-28 — Fase 1: Schema + RLS + helpers](./2026-04-28-fase-1-schema-rls.md)
- [2026-04-28 — Fase 0: Bootstrap (web + backend)](./2026-04-28-fase-0-bootstrap.md)
- [2026-04-28 — Init CLAUDE.md raíz + skills + bitácora](./2026-04-28-init-claude-md-y-skills.md)
